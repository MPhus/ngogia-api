...
import { webValidation } from '~/validations/WebValidation'
...
Router.route('/')
	...
	.post(upload.single('icon'), webController.createNewTest)

Router.route('/token/:token')
	.get(webController.confirmValidEmail)

Router.route('/:slug')
	.get(webController.getWebBySlug)
	// .post(upload.single('icon'), ProductValidation.createNew, productController.createNew)
	.put(upload.single('icon'), webController.updateWeb)

Router.route('/:slug/home')
