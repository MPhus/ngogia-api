...
const createNew = async (data, customerInfor) => {
	...
	try {
		const resuil = await GET_DB().collection(ORDERS_COLLECTION_NAME).insertOne({ ...data })
...
const getMonthList = async (webId) => {
	try {
		...
		const monthsYearObj = orders.map(order => {
			const date = new CustomDate(order.createAt)
			return parseInt(`${date.getMonth() + 1}${date.getFullYear()}`)// getMonth() trả về giá trị từ 0 (tháng 1) đến 11 (tháng 12), nên cộng thêm 1 để chuyển sang tháng thực tế.
		})

		const uniqueMonths = Array.from(new Set(monthsYearObj))
